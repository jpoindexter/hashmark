...
import { readFileSync as readFileSync27, existsSync as existsSync36 } from "fs";
...
`);
  ...
  const claudeBin = candidates.find((p) => {
    try {
      return !p.includes("node_modules") ? existsSync6(p) : existsSync6(p);
    } catch {
      return false;
    }
  }) ?? "claude";
...
function loadAgentContent(projectDir, agentPath) {
  try {
    ...
    if (existsSync6(fullPath)) return readFileSync4(fullPath, "utf-8");
  } catch {
  }
  return null;
}
...
import { existsSync as existsSync7, readFileSync as readFileSync5, writeFileSync as writeFileSync4, mkdirSync as mkdirSync5 } from "fs";
...
function resolveBinPath(bin, projectDir) {
  ...
  if (projectDir) {
    ...
    if (existsSync7(localBin)) return localBin;
  }
  for (const dir of EXTRA_BIN_DIRS) {
    const fullPath = join7(dir, bin);
    if (existsSync7(fullPath)) return fullPath;
  }
  return null;
}
...
import { existsSync as existsSync9, mkdirSync as mkdirSync6, readFileSync as readFileSync6, writeFileSync as writeFileSync5 } from "fs";
...
import { existsSync as existsSync10, unlinkSync as unlinkSync3, statSync as statSync4 } from "fs";
...
function initSnapshots(projectDir) {
  ...
  if (!existsSync11(join11(dir, ".git"))) {
...
import { writeFileSync as writeFileSync7, mkdirSync as mkdirSync8, existsSync as existsSync13, readFileSync as readFileSync9 } from "fs";
...
var activeProcesses2 = /* @__PURE__ */ new Map();
...
setInterval(() => {
  ...
  for (const [sid, lastActive] of sessionLastActivity) {
    if (now - lastActive > SESSION_IDLE_TIMEOUT && activeProcesses2.has(sid)) {
      const proc = activeProcesses2.get(sid);
      try {
        proc?.kill();
      } catch {
      }
      activeProcesses2.delete(sid);
      sessionLastActivity.delete(sid);
    }
  }
  if (activeProcesses2.size > MAX_ACTIVE_SESSIONS) {
    const sorted = [...sessionLastActivity.entries()].sort((a, b) => a[1] - b[1]);
    while (activeProcesses2.size > MAX_ACTIVE_SESSIONS && sorted.length > 0) {
      const [sid] = sorted.shift();
      try {
        activeProcesses2.get(sid)?.kill();
      } catch {
      }
      activeProcesses2.delete(sid);
      sessionLastActivity.delete(sid);
...
function killAllActiveSessions() {
  for (const proc of activeProcesses2.values()) {
    try {
      proc.kill();
    } catch {
    }
  }
  activeProcesses2.clear();
...
function sessionsRoutes() {
  ...
  app.delete("/:id", (c) => {
    ...
    const active = activeProcesses2.get(id);
    if (active) active.kill();
    activeProcesses2.delete(id);
  ...
  app.post("/:id/chat", async (c) => {
    ...
    if (!session) return c.json({ error: "Not found" }, 404);
    if (session.status === "streaming" || activeProcesses2.has(sessionId)) {
...
`));
        ...
        if (useApiStream && activeProvider) {
          ...
          let fullText = "";
          let aborted = false;
          activeProcesses2.set(sessionId, {
            kill: () => {
              aborted = true;
            }
          });
          streamAIResponse({
            ...
            onDone: () => {
              activeProcesses2.delete(sessionId);
          ...
          activeProcesses2.set(sessionId, { kill: () => proc.kill("SIGTERM") });
...
import { existsSync as existsSync27, readFileSync as readFileSync18, statSync as statSync7, mkdirSync as mkdirSync12 } from "fs";
