...
import { existsSync as existsSync4, readFileSync as readFileSync3, symlinkSync as symlinkSync2, lstatSync as lstatSync2, readdirSync as readdirSync2, statSync as statSync3 } from "fs";
        ...
        proc.on("close", (code) => {
          ...
          if (code === 0) {
            try {
              ...
              safeEnqueue(send({ type: "complete", result: { ...result, fileCount: files.length }, delta: null }));
            } catch {
...
function filesRoutes() {
  ...
  app.get("/read", async (c) => {
    ...
    if (!relPath || relPath.includes("\0")) return c.json({ error: "path required" }, 400);
    const fullPath = resolve2(projectDir, relPath);
...
${trailers}` : commit.message;
      try {
        ...
        results.push({ message: commit.message, ok: false, error: String(err) });
      }
    }
  ...
  app.get("/worktrees", async (c) => {
    ...
    try {
      ...
      return c.json({ worktrees: [], error: err instanceof Error ? err.message : "Failed" });
    }
  });
  app.post("/worktrees", async (c) => {
    ...
    try {
      ...
      return c.json({ ok: false, error: err instanceof Error ? err.message : "Failed to create worktree" }, 500);
    }
  });
  app.post("/worktrees/diff", async (c) => {
    ...
    try {
      ...
      return c.json({ files: [], totalAdded: 0, totalRemoved: 0, error: err instanceof Error ? err.message : "Diff failed" });
    }
  });
  app.post("/worktrees/merge", async (c) => {
    ...
    try {
      ...
      return c.json({ ok: false, error: err instanceof Error ? err.message : "Merge failed" }, 500);
    }
  });
  app.post("/worktrees/remove", async (c) => {
    ...
    try {
      ...
      return c.json({ ok: false, error: err instanceof Error ? err.message : "Remove failed" }, 500);
    }
  });
  app.post("/worktrees/pause", async (c) => {
    ...
    try {
      ...
      return c.json({ ok: false, error: err instanceof Error ? err.message : "Pause failed" }, 500);
    }
  });
  app.post("/worktrees/resume", async (c) => {
    ...
    try {
      ...
      return c.json({ ok: false, error: err instanceof Error ? err.message : "Resume failed" }, 500);
    }
  });
