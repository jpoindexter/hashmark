...
PS1=$'\\033]633;A\\007'$PS1$'\\033]633;B\\007'
`;
function setupShellIntegration(shell2) {
  const baseEnv = process.env;
  if (shell2.endsWith("zsh")) {
...
async function spawnPty(projectDir) {
  const shell2 = process.env.SHELL ?? (os.platform() === "win32" ? "cmd.exe" : "/bin/zsh");
  const { env, cleanup } = setupShellIntegration(shell2);
...
${lines}`;
  ...
  app2.get("/git/commit-diff", async (c) => {
...
function mcpRoutes(projectDir) {
  ...
  app2.get("/tools/terminal", async (c) => {
...
function runRoutes(projectDir) {
  ...
  const dataDir = `${projectDir}/.hashmark`;
  app2.get("/status", (c) => c.json({ active: activeRun2 }));
  app2.delete("/", (c) => {
    activeRun2 = false;
...
function createServer(opts) {
  ...
  app2.get("/api/health", (c) => c.json({ ok: true, timestamp: Date.now() }));
  app2.get("/api/info", async (c) => {
  ...
  app2.route("/api/scan", scanRoutes(opts.projectDir));
  app2.route("/api/agents", agentsRoutes(opts.projectDir));
  app2.route("/api/generate", generateRoutes(opts.projectDir));
  app2.route("/api/tasks", tasksRoutes(opts.projectDir));
  app2.route("/api/sessions", sessionsRoutes(opts.projectDir));
  app2.route("/api/files", filesRoutes(opts.projectDir));
  app2.route("/api/workspace", workspaceRoutes(opts.projectDir));
  app2.route("/api/checkpoints", checkpointRoutes(opts.projectDir));
  app2.route("/api/mcp", mcpRoutes(opts.projectDir));
  app2.route("/api/run", runRoutes(opts.projectDir));
  app2.route("/api/swarm", swarmRoutes(opts.projectDir));
  app2.route("/api/company", companyRoutes(opts.projectDir));
  app2.route("/api/drift", driftRoutes(opts.projectDir));
  app2.route("/api/providers", providersRoutes(opts.projectDir));
  app2.route("/api/governance", governanceRoutes(opts.projectDir));
  app2.route("/api/workspaces", workspacesRoutes(globalDataDir, ctx));
  app2.route("/api/config", configRoutes(opts.projectDir));
  app2.route("/api/sandbox", sandboxRoutes(opts.projectDir));
  app2.use(
    "/*",
  ...
  const server2 = serve({ fetch: app2.fetch, port: opts.port, hostname: "localhost" }, () => {
  });
  attachTerminalWS(server2, opts.projectDir);
  return { app: app2, server: server2 };
}
...
function readConfig2() {
  try {
    if (existsSync22(CONFIG_FILE)) {
      const parsed = JSON.parse(readFileSync21(CONFIG_FILE, "utf-8"));
...
function writeConfig2(config) {
  mkdirSync10(CONFIG_DIR, { recursive: true });
  writeFileSync11(CONFIG_FILE, JSON.stringify(config, null, 2));
}
function addToRecent(config, dir) {
...
if (!process.env.HASHMARK_PROJECT_DIR) {
  ...
  if (config.projectDir && existsSync22(config.projectDir)) {
    process.env.HASHMARK_PROJECT_DIR = config.projectDir;
  } else {
    process.env.HASHMARK_PROJECT_DIR = "__unset__";
  }
}
var PROJECT_DIR = process.env.HASHMARK_PROJECT_DIR;
function loadEnvFile(path) {
  if (!existsSync22(path)) return;
  const lines = readFileSync21(path, "utf-8").split("\n");
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith("#")) continue;
    const eq = trimmed.indexOf("=");
...
function saveWindowState() {
  ...
  const windowState = maximized ? { width: bounds.width, height: bounds.height, maximized: true } : { x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height };
  writeConfig2({ ...config, windowState });
}
...
ipcMain.handle("set-project-dir", async (_event, dir) => {
  const config = readConfig2();
  const updated = addToRecent({ ...config, projectDir: dir }, dir);
  writeConfig2(updated);
  process.env.HASHMARK_PROJECT_DIR = dir;
