...
import { join as join23, basename as basename2 } from "path";
...
function migrate(db) {
  ...
  `);
  const cols = db.pragma("table_info(sessions)").map((r) => r.name);
...
function loadProviders(dataDir) {
  ...
  if (!existsSync6(filePath)) return structuredClone(DEFAULT_STORE);
  try {
...
function resolveMcpConfig(projectDir) {
  const candidates = [
    join10(projectDir, ".mcp.json"),
    join10(homedir(), ".claude", "claude_desktop_config.json")
  ];
  for (const filePath of candidates) {
    ...
    try {
      ...
      if (parsed.mcpServers && Object.keys(parsed.mcpServers).length > 0) {
        ...
        const tmpPath = join10(tmpdir(), `studio-mcp-${hash}.json`);
        if (!existsSync9(tmpPath)) {
          writeFileSync6(tmpPath, raw, "utf-8");
        }
        return tmpPath;
      }
    } catch {
...
function findClaudeBin(projectDir) {
  const candidates = [
    join10(projectDir, "node_modules", ".bin", "claude"),
    "/Applications/Conductor.app/Contents/Resources/bin/claude",
    "/usr/local/bin/claude",
    "claude"
  ];
...
function sessionsRoutes(projectDir) {
  ...
  app.get("/config", (c) => {
    ...
    const claudeAvailable = existsSync9(claudeBin) || claudeBin === "claude";
  ...
  app.post("/:id/chat", async (c) => {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, 'user', ?, ?, ?)
    `).run(randomUUID2(), sessionId, body.message, inputEstimate, Date.now());
    ...
    try {
      if (existsSync9(claudeMdPath)) {
        const raw = readFileSync7(claudeMdPath, "utf-8");
...
`));
        ...
        if (useApiStream && activeProvider) {
          ...
          streamAIResponse({
            ...
            onDone: () => {
              ...
              db.prepare(`
                ...
                VALUES (?, ?, 'assistant', ?, ?, ?, ?)
              `).run(randomUUID2(), sessionId, savedText, msgInputEstimate, msgOutputEstimate, Date.now());
              db.prepare(`
                UPDATE sessions
                SET status = 'idle',
            ...
            onError: (err) => {
              ...
              db.prepare(`
                ...
                VALUES (?, ?, 'assistant', ?, ?)
              `).run(randomUUID2(), sessionId, `Error: ${err.message}`, Date.now());
          ...
          const proc = spawn5(claudeBin, claudeArgs, {
            cwd: projectDir,
...
function setupShellIntegration(shell) {
  ...
  if (shell.endsWith("zsh")) {
    ...
    const tmpDir = mkdtempSync(join11(os.tmpdir(), "studio-zsh-"));
    const zshrc = [
    ...
    writeFileSync7(join11(tmpDir, ".zshrc"), zshrc, "utf-8");
    return {
      env: {
        ...baseEnv,
        ZDOTDIR: tmpDir,
  ...
  if (shell.endsWith("bash")) {
    ...
    const tmpDir = mkdtempSync(join11(os.tmpdir(), "studio-bash-"));
    const bashEnvFile = join11(tmpDir, "studio-init.bash");
    const bashrc = [
...
${lines}`;
    ...
    try {
      if (body.paths?.length) {
        for (const p of body.paths) {
          const fullPath = join12(projectDir, p);
  ...
  app.get("/complexity", async (c) => {
    const cachePath = join12(projectDir, ".hashmark", "complexity-cache.json");
    if (!existsSync10(cachePath)) return c.json({ data: null });
    try {
...
import { existsSync as existsSync11, readFileSync as readFileSync8, writeFileSync as writeFileSync8, mkdirSync as mkdirSync6 } from "fs";
import { join as join13 } from "path";
import { spawn as spawn6 } from "child_process";
function getConfigPath(projectDir) {
  return join13(projectDir, ".hashmark", "workspace.json");
}
function readConfig(projectDir) {
  try {
    const p = getConfigPath(projectDir);
    if (existsSync11(p)) return JSON.parse(readFileSync8(p, "utf-8"));
  } catch {
  }
  return {};
}
function writeConfig(projectDir, config) {
  const dir = join13(projectDir, ".hashmark");
...
function checkpointRoutes(projectDir) {
  ...
  app.get("/", async (c) => {
    ...
    const checkpoints = await Promise.all(
      output.trim().split("\n").filter(Boolean).map(async (line) => {
        ...
        let filesChanged = 0;
        try {
          const { stdout: diffStat } = await execFile2(
            "git",
        ...
        let status = "active";
        try {
          const { stdout: mergeBase } = await execFile2(
            "git",
          ...
          try {
            const { stdout: refDate } = await execFile2(
              "git",
  ...
  app.get("/:id/diff", async (c) => {
    ...
    let hash;
    try {
      const { stdout } = await execFile2("git", ["rev-parse", refName], opts);
      hash = stdout.trim();
    } catch {
  ...
  app.post("/", async (c) => {
    ...
    const { stdout: treeHash } = await execFile2("git", ["write-tree"], opts);
    let parentArgs = [];
    try {
      const { stdout: headHash } = await execFile2("git", ["rev-parse", "HEAD"], opts);
      if (headHash.trim()) parentArgs = ["-p", headHash.trim()];
    } catch {
    }
    const { stdout: commitHash } = await execFile2(
      "git",
      ["commit-tree", treeHash.trim(), ...parentArgs, "-m", `studio-checkpoint: ${label}`],
      opts
    );
    await execFile2("git", ["update-ref", refName, commitHash.trim()], opts);
  ...
  app.post("/:id/restore", async (c) => {
    ...
    const { stdout: objType } = await execFile2("git", ["cat-file", "-t", hash], opts);
    if (objType.trim() !== "commit") {
      return c.json({ error: "ref is not a commit" }, 400);
    }
    const branchName = `restore/${id}`;
    await execFile2("git", ["branch", "-f", branchName, hash], opts);
  ...
  app.delete("/:refSlug", async (c) => {
    ...
    await execFile2("git", ["update-ref", "-d", refName], opts);
    return c.json({ ok: true });
  });
  app.delete("/prune", async (c) => {
    ...
    for (const line of lines) {
      ...
      try {
        await execFile2("git", ["merge-base", "--is-ancestor", hash, "HEAD"], opts);
        shouldPrune = true;
      } catch {
        shouldPrune = true;
      }
  ...
  app.post("/restore", async (c) => {
    const body = await c.req.json();
    const ref = body.ref;
    const { stdout: objType } = await execFile2("git", ["cat-file", "-t", ref], opts);
    if (objType.trim() !== "commit") {
      return c.json({ error: "ref is not a commit" }, 400);
    }
    await execFile2("git", ["checkout", ref, "--", "."], opts);
    return c.json({ ok: true });
  });
  return app;
}
...
function mcpRoutes(projectDir) {
  ...
  app.get("/config", (c) => {
    ...
    const globalMcpPath = join14(homedir2(), ".claude", "claude_desktop_config.json");
  ...
  app.post("/test", async (c) => {
    ...
    const tmpPath = join14(tmpdir2(), `studio-mcp-test-${hash}.json`);
...
import { spawn as spawn8, execFile as execFileCb2 } from "child_process";
import { existsSync as existsSync13, readFileSync as readFileSync10, readdirSync as readdirSync2 } from "fs";
...
function logAgentAction(dataDir, event) {
  try {
    ...
    appendFileSync(join15(dataDir, "agent-actions.jsonl"), line);
  } catch {
  }
}
...
function loadAgents(projectDir) {
  const agentsDir = join16(projectDir, ".claude", "agents");
  if (!existsSync13(agentsDir)) return [];
  const agents = [];
  function walk(dir) {
    let entries;
    try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const relPath = relative3(agentsDir, fullPath);
        try {
...
function findClaudeBin2(projectDir) {
  const candidates = [
    join16(projectDir, "node_modules", ".bin", "claude"),
    "/Applications/Conductor.app/Contents/Resources/bin/claude",
    "/usr/local/bin/claude",
    "claude"
  ];
...
[{"id":1,"title":"short title under 50 chars","description":"detailed what to implement","agentId":"exact-agent-id-here"}]`;
    try {
      const { stdout } = await execFile3(claudeBin, ["--print", prompt], {
        cwd: projectDir,
        env: { ...process.env, CLAUDE_DANGEROUSLY_SKIP_PERMISSIONS: "1" },
        maxBuffer: 1024 * 1024
      });
...
`));
        ...
        async function runWorker(subtask) {
          ...
          const worktreeDir = join16(tmpdir3(), branchName);
          ...
          try {
            await execFile3("git", ["worktree", "add", worktreeDir, "-b", branchName], {
              cwd: projectDir
            });
...
Work in the current directory. Make the necessary code changes, create or modify files as needed.`;
          return new Promise((resolve, reject) => {
            ...
            proc.on("close", async (code) => {
              ...
              try {
                const { stdout: statusOut } = await execFile3("git", ["status", "--porcelain"], { cwd: worktreeDir });
                hasChanges = statusOut.trim().length > 0;
                if (hasChanges) {
                  await execFile3("git", ["add", "-A"], { cwd: worktreeDir });
                  await execFile3("git", ["commit", "-m", `feat(swarm/${runId}): agent ${subtask.id} - ${subtask.title}`], { cwd: worktreeDir });
              ...
              try {
                const pkgPath = join16(worktreeDir, "package.json");
                ...
                if (hasTestScript) {
                  ...
                  const { stdout: testOut, stderr: testErr } = await execFile3(
                    "npm",
        ...
        async function orchestrate() {
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            try {
              ...
              try {
                await execFile3("git", ["merge", "--abort"], { cwd: projectDir });
              } catch {
              }
              conflicts.push(subtask.id);
          ...
          for (const subtask of plan) {
            ...
            try {
              if (worktreeDir) {
                await execFile3("git", ["worktree", "remove", worktreeDir, "--force"], { cwd: projectDir });
            ...
            try {
              await execFile3("git", ["branch", "-D", branchName], { cwd: projectDir });
            } catch {
            }
          }
          try {
...
import { spawn as spawn9, execFile as execFileCb3 } from "child_process";
import { existsSync as existsSync14, readFileSync as readFileSync11, readdirSync as readdirSync3 } from "fs";
import { join as join17, relative as relative4 } from "path";
import { randomUUID as randomUUID4 } from "crypto";
import { promisify as promisify4 } from "util";
import { tmpdir as tmpdir4 } from "os";
var execFile4 = promisify4(execFileCb3);
function loadAgents2(projectDir) {
  const agentsDir = join17(projectDir, ".claude", "agents");
  if (!existsSync14(agentsDir)) return [];
  const agents = [];
  function walk(dir) {
    let entries;
    try {
...
function findClaudeBin3(projectDir) {
  const candidates = [
    join17(projectDir, "node_modules", ".bin", "claude"),
    "/Applications/Conductor.app/Contents/Resources/bin/claude",
    "/usr/local/bin/claude",
    "claude"
  ];
...
function runRoutes(projectDir) {
  ...
  app.get("/runs/:id/diff", async (c) => {
    ...
    try {
      ...
      let diff = "";
      try {
        const res = await execFile4("git", ["diff", `main...${branch}`], {
          cwd: projectDir,
        ...
        try {
          ...
          if (hash) {
            const showRes = await execFile4("git", ["show", hash], {
              cwd: projectDir,
              maxBuffer: 4 * 1024 * 1024
            });
            diff = showRes.stdout;
...
`));
        ...
        const worktreeDir = join17(tmpdir4(), branchName);
        async function run() {
          ...
          try {
            await execFile4("git", ["worktree", "add", worktreeDir, "-b", branchName], {
              cwd: projectDir
            });
...
Work in the current directory. Make the necessary code changes, create or modify files as needed.`;
          ...
          if (mode === "plan") {
            ...
            controller.close();
            try {
              await execFile4("git", ["worktree", "remove", worktreeDir, "--force"], { cwd: projectDir });
            } catch {
            }
            try {
              await execFile4("git", ["branch", "-D", branchName], { cwd: projectDir });
            } catch {
            }
            return;
          }
          ...
          for (const ev of actionEvents) logAgentAction(dataDir, ev);
          try {
            const { stdout: statusOut } = await execFile4("git", ["status", "--porcelain"], { cwd: worktreeDir });
            hasChanges = statusOut.trim().length > 0;
            if (hasChanges) {
              await execFile4("git", ["add", "-A"], { cwd: worktreeDir });
              await execFile4("git", ["commit", "-m", `feat(run/${runId}): ${body.task.slice(0, 72)}`], { cwd: worktreeDir });
...
import { spawn as spawn10, execFile as execFileCb4 } from "child_process";
import { existsSync as existsSync15, readFileSync as readFileSync12, readdirSync as readdirSync4 } from "fs";
import { join as join18, relative as relative5 } from "path";
...
function findClaudeBin4(projectDir) {
  const candidates = [
    join18(projectDir, "node_modules", ".bin", "claude"),
    "/Applications/Conductor.app/Contents/Resources/bin/claude",
    "/usr/local/bin/claude",
    "claude"
  ];
...
async function runAgent(swarm, agentIndex, projectDir, dataDir, claudeBin, agentMap) {
  ...
  const { branch } = agent;
  const worktreeDir = join18(tmpdir5(), branch);
  ...
  try {
    await execFile5("git", ["worktree", "add", worktreeDir, "-b", branch], { cwd: projectDir });
...
[spawn error] ${err.message}`;
  ...
  if (swarm.cancelled || ctrl.signal.aborted) {
    ...
    try {
      await execFile5("git", ["branch", "-D", branch], { cwd: projectDir });
    } catch {
    }
    return;
  }
  ...
  let hasChanges = false;
  try {
    const { stdout } = await execFile5("git", ["status", "--porcelain"], { cwd: worktreeDir });
...
function driftRoutes(projectDir) {
  ...
  app.get("/check", async (c) => {
    ...
    for (const name of CONTEXT_FILES) {
      try {
        content = await readFile2(join19(projectDir, name), "utf-8");
        fileName = name;
        break;
      } catch {
      }
    ...
    if (ageDays === null) {
      try {
        const s = await stat2(join19(projectDir, fileName));
...
function governanceRoutes(dataDir) {
  ...
  app.get("/policies", (c) => {
    ...
    return c.json({ policies: policies.map((p) => ({ ...p, rules: JSON.parse(p.rules) })) });
  });
  ...
  app.get("/action-log", (c) => {
    const logPath = join20(dataDir, "agent-actions.jsonl");
    if (!existsSync16(logPath)) return c.json({ events: [], total: 0 });
    const limit = parseInt(c.req.query("limit") ?? "100");
    const offset = parseInt(c.req.query("offset") ?? "0");
...
import { existsSync as existsSync17, readFileSync as readFileSync14, statSync as statSync2 } from "fs";
import { join as join21, basename } from "path";
function readProjectName(projectDir) {
  const pkgPath = join21(projectDir, "package.json");
  try {
...
function workspacesRoutes(globalDataDir, ctx) {
  ...
  app.post("/:id/activate", (c) => {
    ...
    if (!existsSync17(ws.path)) return c.json({ error: "workspace path no longer exists" }, 400);
...
import { existsSync as existsSync18, readFileSync as readFileSync15, writeFileSync as writeFileSync10, mkdirSync as mkdirSync8 } from "fs";
...
function loadConfig(dataDir) {
  const filePath = join22(dataDir, "scan-config.json");
  if (!existsSync18(filePath)) return { ...DEFAULT_CONFIG };
  try {
...
function saveConfig(dataDir, config) {
  if (!existsSync18(dataDir)) mkdirSync8(dataDir, { recursive: true });
  writeFileSync10(join22(dataDir, "scan-config.json"), JSON.stringify(config, null, 2), "utf-8");
}
function configRoutes(projectDir) {
  const app = new Hono17();
  const dataDir = join22(projectDir, ".hashmark");
  app.get("/", (c) => {
...
function createServer(opts) {
  ...
  if (opts.projectDir !== "__unset__") {
    try {
      const db = getDb(globalDataDir);
      const pkgPath = join23(opts.projectDir, "package.json");
  ...
  app.get("*", (c) => {
    const indexPath = join23(opts.staticDir, "index.html");
    if (existsSync19(indexPath)) {
      const html = readFileSync16(indexPath, "utf-8");
      return c.html(html);
    }
    return c.html(`
      <!DOCTYPE html>
