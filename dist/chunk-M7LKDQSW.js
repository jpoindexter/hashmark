...
import { readFileSync as readFileSync27, existsSync as existsSync36, statfsSync } from "fs";
import { join as join40, basename as basename8 } from "path";
import { randomUUID as randomUUID18 } from "crypto";

// server/routes/agents.ts
import { Hono } from "hono";
import { readFileSync, readdirSync, existsSync as existsSync2, writeFileSync, unlinkSync as unlinkSync2, mkdirSync as mkdirSync2, renameSync, statSync } from "fs";
import { join as join2, relative } from "path";

// server/db.ts
import Database from "better-sqlite3";
import { join } from "path";
import { mkdirSync, chmodSync, existsSync, copyFileSync, unlinkSync } from "fs";
...
function migrateV1(db) {
  db.exec(`
    ...
    -- NOTE: idx_sessions_archived deferred to v2 where 'archived' column is added
...
function migrateV2(db) {
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_sessions_archived ON sessions(archived, updated_at)");
...
function migrateV8(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_agent_actions_outcome ON agent_actions(outcome, created_at);
    CREATE INDEX IF NOT EXISTS idx_agent_actions_created_at ON agent_actions(created_at);
    CREATE INDEX IF NOT EXISTS idx_reviews_session ON reviews(session_id);
    CREATE INDEX IF NOT EXISTS idx_messages_role ON session_messages(session_id, role, sent_at);
  `);
}
function migrateV9(db) {
  const cols = colNames(db, "runs");
  if (!cols.includes("task")) {
...
function attemptDbRecovery(dataDir) {
  const dbPath = join(dataDir, "studio.db");
  const backupPath = join(dataDir, `studio.db.corrupt.${Date.now()}`);
  try {
    ...
    if (existsSync(dbPath)) {
      ...
      console.error(`[db] corrupt DB backed up to ${backupPath}`);
    }
    try {
      const corruptDb = new Database(dbPath, { readonly: true });
      const recoveryPath = join(dataDir, "studio.db.recovery");
      const recoveryDb = new Database(recoveryPath);
      ...
      for (const table of tables) {
        try {
          ...
          if (rows.length > 0) {
            ...
            const insert = recoveryDb.prepare(
              `INSERT OR IGNORE INTO "${table.name}" (${cols.join(", ")}) VALUES (${placeholders})`
            );
            const bulkInsert = recoveryDb.transaction((data) => {
              for (const row of data) insert.run(...cols.map((c) => row[c]));
            });
            bulkInsert(rows);
          }
        } catch {
          console.error(`[db] recovery: failed to recover table ${table.name}`);
        }
      }
      corruptDb.close();
      recoveryDb.close();
      ...
      console.error("[db] recovery succeeded, data reimported from corrupt backup");
      return true;
    } catch (recoveryErr) {
      console.error("[db] recovery via reimport failed:", recoveryErr);
      try {
        unlinkSync(dbPath);
      } catch {
      }
      ...
      console.error("[db] started fresh database (corrupt backup preserved)");
      return true;
    }
  } catch (err) {
    console.error("[db] recovery failed completely:", err);
    return false;
  }
}
function getDb(dataDir) {
  ...
  try {
    ...
    if (check[0]?.quick_check !== "ok") {
      console.error("[db] integrity check failed:", check[0]?.quick_check);
      db.close();
      if (attemptDbRecovery(dataDir)) {
        db = new Database(dbPath);
        db.pragma("journal_mode = WAL");
        ...
        throw new Error("Database corrupt and recovery failed");
      }
    }
  } catch (err) {
    if (err instanceof Error && err.message === "Database corrupt and recovery failed") throw err;
    console.error("[db] integrity check threw:", err);
  }
  runMigrations(db);
  _dbs.set(dataDir, db);
  return db;
...
`;
  ...
  app.post("/import", async (c) => {
    ...
    if (!targetPath.startsWith(agentsDir + "/") && targetPath !== agentsDir) {
...
import { mkdirSync as mkdirSync3, writeFileSync as writeFileSync2, existsSync as existsSync3, symlinkSync, lstatSync } from "fs";
...
function generateRoutes() {
  ...
  app.post("/", async (c) => {
    ...
    if (!body.companyType?.trim()) return c.json({ error: "companyType required" }, 400);
    let proc = null;
...
`;
        ...
        const isMonorepo = !existsSync3(localBin) && existsSync3(monoBin);
        const resolvedBin = existsSync3(localBin) ? localBin : isMonorepo ? "node" : "hashmark";
        if (resolvedBin === "node") {
        ...
        proc = spawn(resolvedBin, args, { cwd: spawnCwd, env, stdio: ["pipe", "pipe", "pipe"] });
  ...
  app.post("/save", async (c) => {
    ...
    if (!Array.isArray(body.agents) || body.agents.length === 0) {
      return c.json({ error: "agents array required" }, 400);
    }
...
import { existsSync as existsSync5, readFileSync as readFileSync3, writeFileSync as writeFileSync3, mkdirSync as mkdirSync4, symlinkSync as symlinkSync2, lstatSync as lstatSync2, readdirSync as readdirSync2, statSync as statSync3 } from "fs";
...
function scanRoutes() {
  ...
  app.get("/history", (c) => {
    ...
    if (!existsSync5(snapshotPath)) return c.json({ snapshots: [] });
    try {
...
`;
    ...
    const stream = new ReadableStream({
      ...
      start(controller) {
        ...
        const isMonorepo = !existsSync5(localBin) && existsSync5(monoBin);
        const bin = existsSync5(localBin) ? localBin : isMonorepo ? "node" : "hashmark";
...
function resolveAgentBinary(name) {
  ...
  if (resourcesPath) {
    const bundled = join6(resourcesPath, "bin", name);
    if (existsSync6(bundled)) return bundled;
  }
  return name;
}
...
function findBin(name, projectDir) {
  const candidates = [
    ...
    join7(projectDir, "node_modules", ".bin", name),
...
function loadAgentContent(projectDir, agentPath) {
  try {
    const fullPath = join8(projectDir, ".claude", "agents", agentPath);
    if (existsSync8(fullPath)) return readFileSync5(fullPath, "utf-8");
  } catch {
  }
  return null;
}
...
function tasksRoutes() {
  ...
  app.post("/", async (c) => {
    ...
    if (!body.agentName?.trim()) return c.json({ error: "agentName required" }, 400);
...
function detectToolObsession(msgs) {
  ...
  const windowStart = Math.max(0, msgs.length - 10);
...
import { existsSync as existsSync9, readFileSync as readFileSync6, writeFileSync as writeFileSync4, mkdirSync as mkdirSync5 } from "fs";
...
function resolveBinPath(bin, projectDir) {
  ...
  if (projectDir) {
    const localBin = join9(projectDir, "node_modules", ".bin", bin);
    if (existsSync9(localBin)) return localBin;
  }
  for (const dir of EXTRA_BIN_DIRS) {
    const fullPath = join9(dir, bin);
    if (existsSync9(fullPath)) return fullPath;
  }
  return null;
}
...
function loadProviders(dataDir) {
  const filePath = join9(dataDir, "providers.json");
...
function saveProviders(dataDir, store) {
  if (!existsSync9(dataDir)) mkdirSync5(dataDir, { recursive: true });
  const filePath = join9(dataDir, "providers.json");
  const encrypted = {
    ...store,
...
async function streamCodex(opts) {
  const candidates = [
    join10(process.cwd(), "node_modules", ".bin", "codex"),
    "/usr/local/bin/codex",
    "codex"
  ];
  const codexBin = candidates.find((p) => {
    try {
      return existsSync10(p);
    } catch {
      return false;
    }
  }) ?? "codex";
...
function analyticsPath(dataDir, sessionId) {
  return join11(dataDir, "analytics", `${sessionId}.json`);
}
function ensureDir(dataDir) {
  const dir = join11(dataDir, "analytics");
  if (!existsSync11(dir)) {
...
import { existsSync as existsSync12, unlinkSync as unlinkSync3, statSync as statSync4 } from "fs";
...
async function createCheckpoint(projectDir, label) {
  ...
  try {
    const lockPath = join12(projectDir, ".git", "index.lock");
...
function shadowDir(projectDir) {
  return join13(projectDir, ".hashmark", "snapshots");
}
function indexPath(projectDir) {
  return join13(shadowDir(projectDir), "index.json");
}
...
function initSnapshots(projectDir) {
  ...
  if (!existsSync13(join13(dir, ".git"))) {
...
async function syncToShadow(projectDir) {
  ...
  for (const entry of entries) {
    ...
    const src = join13(projectDir, entry.name);
    const dest = join13(shadow, entry.name);
    try {
...
function loadProjectEnvVars(projectDir) {
  ...
  for (const fname of [".env", ".env.local"]) {
    const filePath = join14(projectDir, fname);
...
import { writeFileSync as writeFileSync7, mkdirSync as mkdirSync8, existsSync as existsSync15, readFileSync as readFileSync10 } from "fs";
...
function createStudioMcpConfig(projectDir, port) {
  ...
  const configDir = join15(tmpdir(), "hashmark-studio-mcp");
  mkdirSync8(configDir, { recursive: true, mode: 448 });
  const scriptPath = join15(configDir, `bridge-${port}.js`);
  ...
  const configPath = join15(configDir, `mcp-config-${hash}.json`);
...
function collectUserMcpServers(projectDir) {
  const merged = {};
  const candidates = [
    join15(homedir(), ".claude", "claude_desktop_config.json"),
    join15(projectDir, ".mcp.json")
  ];
  for (const filePath of candidates) {
    if (!existsSync15(filePath)) continue;
    try {
...
async function pauseWorktree(projectDir, worktreePath, branch) {
  if (!existsSync16(worktreePath)) return;
  const opts = { cwd: worktreePath };
  try {
...
var _evictionTimer = setInterval(() => {
  const now = Date.now();
...
function clearEvictionInterval() {
  if (_evictionTimer) {
...
function sessionsRoutes() {
  ...
  app.get("/config", (c) => {
    const { projectDir } = c.get("project");
    const claudeBin = findClaudeBin(projectDir);
    const claudeAvailable = existsSync17(claudeBin) || claudeBin === "claude";
  ...
  app.post("/:id/chat", async (c) => {
    ...
    if (!session) return c.json({ error: "Not found" }, 404);
    if (session.status === "streaming" || activeProcesses.has(sessionId)) {
      return c.json({ error: "Session already has an active stream. Interrupt it first." }, 409);
    }
...
`));
        ...
        const _markSentStmt = db.prepare("UPDATE session_messages SET sent_at = ? WHERE id = ?");
        const markMessagesSent = () => {
          if (messageMarkedSent) return;
          messageMarkedSent = true;
          const now = Date.now();
          const markAll = db.transaction(() => {
            _markSentStmt.run(now, userMsgId);
            for (const pm of pendingMessages) {
              _markSentStmt.run(now, pm.id);
            }
          });
          markAll();
        };
  ...
  app.post("/:id/fork", async (c) => {
    ...
    `);
    db.transaction(() => {
      insertSession.run(newId, newTitle, source.agent_id, source.agent_name, source.model, now, now);
...
${trailers}` : commit.message;
  ...
  app.get("/complexity", async (c) => {
    ...
    if (!existsSync18(cachePath)) return c.json({ data: null });
    try {
  ...
  app.post("/git/branch", async (c) => {
    ...
    try {
      await execAsync("git", ["switch", "-c", name], { cwd: projectDir });
  ...
  app.post("/git/checkout", async (c) => {
    ...
    try {
      await execAsync("git", ["switch", branch], { cwd: projectDir });
      return c.json({ ok: true });
    } catch (err) {
  ...
  app.get("/git", async (c) => {
    ...
    try {
      ...
      const stagedStats = /* @__PURE__ */ new Map();
      const unstagedStats = /* @__PURE__ */ new Map();
      try {
        const { stdout: stagedRaw } = await execAsync("git", ["diff", "--numstat", "--cached"], { cwd: projectDir });
        for (const line of stagedRaw.trim().split("\n").filter(Boolean)) {
          const parts = line.split("	");
          if (parts.length === 3) {
            stagedStats.set(parts[2].trim(), { added: parseInt(parts[0]) || 0, removed: parseInt(parts[1]) || 0 });
          }
        }
      } catch {
      }
      try {
        const { stdout: unstagedRaw } = await execAsync("git", ["diff", "--numstat", "HEAD"], { cwd: projectDir });
        for (const line of unstagedRaw.trim().split("\n").filter(Boolean)) {
          const parts = line.split("	");
          if (parts.length === 3) {
            unstagedStats.set(parts[2].trim(), { added: parseInt(parts[0]) || 0, removed: parseInt(parts[1]) || 0 });
          }
        }
      } catch {
      }
      const filesWithStats = rawFiles.map((f) => {
        const stats = f.isStaged ? stagedStats.get(f.file) : unstagedStats.get(f.file);
  ...
  const safeGitRef = (ref) => /^[a-zA-Z0-9._/\-]+$/.test(ref) && !ref.startsWith("-");
  const safeSha = (sha) => /^[0-9a-fA-F]{4,64}$/.test(sha);
  app.post("/worktrees/diff", async (c) => {
    const body = await c.req.json();
    if (!body.worktreePath || !body.baseCommitSHA) return c.json({ error: "worktreePath and baseCommitSHA required" }, 400);
    if (!safeSha(body.baseCommitSHA)) return c.json({ error: "invalid commit SHA" }, 400);
    try {
  ...
  app.post("/worktrees/merge", async (c) => {
    const { projectDir } = c.get("project");
    const body = await c.req.json();
    if (!body.branch || !safeGitRef(body.branch)) return c.json({ error: "invalid branch name" }, 400);
    try {
...
import { existsSync as existsSync19, readFileSync as readFileSync12, writeFileSync as writeFileSync9, mkdirSync as mkdirSync10 } from "fs";
...
function readConfig(projectDir) {
  try {
    ...
    if (existsSync19(p)) return JSON.parse(readFileSync12(p, "utf-8"));
  } catch {
  }
  return {};
}
...
function workspaceRoutes() {
  ...
  app.post("/detect", async (c) => {
    ...
    if (!existsSync19(dir)) return c.json({ error: "path not found" }, 400);
    const name = (() => {
      try {
    ...
    for (const check of checks) {
      if (existsSync19(join20(dir, check.file))) {
...
import { existsSync as existsSync20, unlinkSync as unlinkSync4, statSync as statSync6 } from "fs";
  ...
  app.post("/", async (c) => {
    ...
    if (existsSync20(lockPath)) {
      try {
        ...
        if (age > 3e4) unlinkSync4(lockPath);
      } catch {
      }
    }
    try {
...
DIRTY_BASELINE:${JSON.stringify(dirtyBaseline)}`;
  ...
  app.delete("/:refSlug", async (c) => {
    ...
    try {
      ...
      return c.json({ error: "checkpoint not found or already deleted" }, 404);
    }
  });
...
import { existsSync as existsSync21, readFileSync as readFileSync13, writeFileSync as writeFileSync10 } from "fs";
...
import { existsSync as existsSync22, readFileSync as readFileSync14, readdirSync as readdirSync3 } from "fs";
...
import { existsSync as existsSync23, readFileSync as readFileSync15, readdirSync as readdirSync4 } from "fs";
...
import { existsSync as existsSync24, readFileSync as readFileSync16, readdirSync as readdirSync5 } from "fs";
...
function loadAgents3(projectDir) {
  ...
  if (!existsSync24(agentsDir)) return [];
  const agents = [];
  function walk(dir) {
    let entries;
    try {
...
function swarmRoutes() {
  ...
  app.post("/", async (c) => {
    ...
    try {
      ...
      db.transaction(() => {
        insertRun.run(swarmId, mode, Date.now());
        for (const agent of agents) {
          insertAgent.run(agent.id, swarmId, agent.task, agent.agentId ?? null, agent.branch, Date.now());
        }
      })();
    } catch {
    }
...
`));
        ...
        const s = swarm;
        s._listeners ??= [];
        const listener = (agentIndex, event) => {
        ...
        const cleanup = () => {
          ...
          const idx = s._listeners?.indexOf(listener) ?? -1;
          if (idx >= 0) s._listeners.splice(idx, 1);
          try {
            controller.close();
          } catch {
          }
...
function governanceRoutes() {
  ...
  app.post("/actions", async (c) => {
    ...
    if (!body.actionType?.trim()) return c.json({ error: "actionType required" }, 400);
    const db = getDb(dataDir);
    db.prepare(
  ...
  app.get("/summary", (c) => {
    ...
    return c.json({ total: counts.total, blocked: counts.blocked, flagged: counts.flagged, byType, recentBlocked });
  });
  app.get("/action-log", (c) => {
    const { dataDir } = c.get("project");
    const logPath = join29(dataDir, "agent-actions.jsonl");
    if (!existsSync26(logPath)) return c.json({ events: [], total: 0 });
    try {
      const fileSize = statSync7(logPath).size;
      if (fileSize > 10 * 1024 * 1024) {
        return c.json({ events: [], total: 0, error: "Log file too large" });
      }
    } catch {
    }
    const limit = Math.min(Math.max(parseInt(c.req.query("limit") ?? "100") || 100, 1), 500);
...
import { existsSync as existsSync27, readFileSync as readFileSync18, statSync as statSync8, mkdirSync as mkdirSync12 } from "fs";
...
function workspacesRoutes(globalDataDir) {
  ...
  app.post("/:id/activate", (c) => {
    ...
    if (!existsSync27(ws.path)) return c.json({ error: "workspace path no longer exists" }, 400);
    db.transaction(() => {
...
import { existsSync as existsSync28, readFileSync as readFileSync19, writeFileSync as writeFileSync11, mkdirSync as mkdirSync13 } from "fs";
...
function loadConfig(dataDir) {
  ...
  if (!existsSync28(filePath)) return { ...DEFAULT_CONFIG2 };
  try {
...
function saveConfig(dataDir, config) {
  if (!existsSync28(dataDir)) mkdirSync13(dataDir, { recursive: true });
...
function reviewsRoutes() {
  ...
  app.post("/", async (c) => {
    ...
    for (const policy of policies) {
      ...
      if (policy.rule_type === "path_pattern") {
        try {
          const escaped = policy.pattern.replace(/[.+?^${}()|[\]\\]/g, "\\$&").replace(/\*/g, ".*");
    ...
    db.transaction(() => {
      ...
      db.prepare("INSERT INTO review_events (review_id, event_type, author, data, created_at) VALUES (?, ?, ?, ?, ?)").run(id, "created", body.agent_name, JSON.stringify({ file_path: body.file_path }), Date.now());
    })();
  ...
  app.patch("/policies/:id", async (c) => {
    ...
    if (fields.length > 0) {
      values.push(id);
      db.prepare(`UPDATE review_policies SET ${fields.join(", ")} WHERE id = ?`).run(...values);
    }
    return c.json({ ok: true });
  });
  app.delete("/policies/:id", (c) => {
  ...
  app.post("/test/:provider", async (c) => {
    ...
    if (provider === "github") {
      try {
        ...
        if (!res.ok) return c.json({ ok: false, error: `GitHub API returned ${res.status}` }, 502);
        ...
        return c.json({ ok: false, error: err instanceof Error ? err.message : "Test failed" }, 502);
      }
    }
    if (provider === "linear") {
      try {
        ...
        if (!res.ok) return c.json({ ok: false, error: `Linear API returned ${res.status}` }, 502);
        const data = await res.json();
...
import { existsSync as existsSync30, readFileSync as readFileSync21, readdirSync as readdirSync6 } from "fs";
...
function detectTestCommand(projectDir) {
  ...
  if (!existsSync31(pkgPath)) return null;
  try {
...
function settingsRoutes(defaultProjectDir) {
  ...
  app.get("/env", async (c) => {
    ...
    for (const fname of [".env.local", ".env"]) {
      ...
      if (!existsSync32(filePath)) continue;
      try {
...
import { existsSync as existsSync33, readFileSync as readFileSync24, writeFileSync as writeFileSync12, mkdirSync as mkdirSync14 } from "fs";
...
function readCache() {
  try {
    if (!existsSync33(CACHE_FILE)) return null;
...
function writeCache(reg) {
  try {
    if (!existsSync33(CACHE_DIR)) mkdirSync14(CACHE_DIR, { recursive: true });
...
import { readFileSync as readFileSync25, writeFileSync as writeFileSync13, mkdirSync as mkdirSync15, existsSync as existsSync34, unlinkSync as unlinkSync5 } from "fs";
...
function saveTokens(tokens, email) {
  ...
  if (!existsSync34(dir)) mkdirSync15(dir, { recursive: true });
  const data = { ...tokens, email };
  writeFileSync13(tokenPath(), JSON.stringify(data, null, 2), { mode: 384 });
}
function loadTokens() {
  try {
    if (!existsSync34(tokenPath())) return null;
...
function clearTokens() {
  try {
    if (existsSync34(tokenPath())) unlinkSync5(tokenPath());
  } catch {
  }
}

// server/lib/oauth-copilot.ts
import { existsSync as existsSync35, readFileSync as readFileSync26, writeFileSync as writeFileSync14, mkdirSync as mkdirSync16, unlinkSync as unlinkSync6 } from "fs";
...
function saveCopilotAuth(dataDir, auth) {
  if (!existsSync35(dataDir)) mkdirSync16(dataDir, { recursive: true });
...
function deleteCopilotAuth(dataDir) {
  const path = authFilePath(dataDir);
  if (existsSync35(path)) unlinkSync6(path);
}
...
function createServer(opts) {
  ...
  app.get("/api/health", (c) => {
    ...
    let healthy = true;
    try {
      const project = registry.active;
      if (project) {
        const db = getDb(project.dataDir);
        const row = db.prepare("SELECT 1 as v").get();
        checks.db = { ok: row?.v === 1 };
        if (!checks.db.ok) healthy = false;
      } else {
        checks.db = { ok: true, detail: "no project connected" };
      }
    } catch (err) {
    ...
    const processCounts = {
      sessions: activeProcesses.size,
      projects: registry.list().length
    };
    checks.processes = { ok: true, detail: `${processCounts.sessions} sessions, ${processCounts.projects} projects` };
    try {
      const project = registry.active;
      if (project) {
        const stats = statfsSync(project.dataDir);
        const freeBytes = stats.bavail * stats.bsize;
        const freeMB = Math.round(freeBytes / (1024 * 1024));
        const diskOk = freeMB > 100;
        checks.disk = { ok: diskOk, detail: `${freeMB}MB free` };
        if (!diskOk) healthy = false;
      }
    } catch {
      checks.disk = { ok: true, detail: "check unavailable" };
    }
    return c.json({
      ok: healthy,
      timestamp: Date.now(),
  ...
  app.get("*", (c) => {
    ...
    if (existsSync36(indexPath2)) return c.html(readFileSync27(indexPath2, "utf-8"));
  ...
  app.onError((err, c) => {
    console.error(`[server] route error ${c.req.method} ${c.req.path}:`, err);
