...
/** Standalone drift check -- returns data without Hono context */
export async function checkDrift(projectDir: string): Promise<DriftResult | NoDriftResult> {
  ...
  if (baselineCount !== null && currentFileCount !== null && baselineCount > 0) {
    ...
    if (delta >= 5) {
      signals.push({ type: "file_count_delta", current: currentFileCount, baseline: baselineCount, delta });
    }
  }
  ...
  try {
    const { stdout } = await execAsync("git", ["log", "-1", "--format=%ct", "--", fileName], { cwd: projectDir });
  ...
  return { hasContextFile: true, fileName, driftLevel, signals, recommendation };
}
...
export function driftRoutes(ctx: WorkspaceCtx) {
  ...
  app.get("/check", async (c) => {
    const result = await checkDrift(ctx.projectDir);
